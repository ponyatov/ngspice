...
ft_numparse(char **s, bool whole)
{
    ...
    if ((!isdigit(char_to_int(*string)) && *string != '.') ||
        ((*string == '.') && !isdigit(char_to_int(string[1]))))
        return (NULL);

    /* Now accumulate a number. Note ascii dependencies here... */
    while (isdigit(char_to_int(*string)))
    ...
    if (whole && *string != '\0') {
        return (NULL);
    } else if (ft_strictnumparse && *string && isdigit(char_to_int(string[-1]))) {
        if (*string == '_')
            while (isalpha(char_to_int(*string)) || (*string == '_'))
                string++;
        else
            return (NULL);
    } else {
