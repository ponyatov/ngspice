...
static char *skip_back_non_ws(char *d) { while (d[-1] && !isspace(char_to_int(d[-1]))) d--; return d; }
static char *skip_back_ws(char *d)     { while (isspace(char_to_int(d[-1])))           d--; return d; }
static char *skip_non_ws(char *d)      { while (*d && !isspace(char_to_int(*d))) d++; return d; }
static char *skip_ws(char *d)          { while (isspace(char_to_int(*d)))        d++; return d; }

static char *skip_back_non_ws_(char *d, char *start) { while (d > start && !isspace(char_to_int(d[-1]))) d--; return d; }
static char *skip_back_ws_(char *d, char *start)     { while (d > start && isspace(char_to_int(d[-1]))) d--; return d; }
...
find_section_definition(struct line *c, char *name)
{
    for (; c; c = c->li_next) {
        ...
        if (ciprefix(".lib", line)) {
            ...
            s = skip_non_ws(line);
            while (isspace(char_to_int(*s)) || isquote(*s))
                s++;
            for (t = s; *t && !isspace(char_to_int(*t)) && !isquote(*t); t++)
                ;
            y = t;
            while (isspace(char_to_int(*y)) || isquote(*y))
                y++;
...
*/
{
    ...
    for (;;) {
        ...
        if (ciprefix(".end", buffer))
            if ((buffer[4] == '\0') || isspace(char_to_int(buffer[4]))) {
                found_end = TRUE;
                *buffer   = '*';
            }
...
inp_pathresolve(const char *name)
{
    ...
    if (cp_getvar("mingwpath", CP_BOOL, NULL) && name[0] == DIR_TERM_LINUX && isalpha(char_to_int(name[1])) && name[2] == DIR_TERM_LINUX) {
        strcpy(buf, name);
        buf[0] = buf[1];
        buf[1] = ':';
...
inp_fix_gnd_name(struct line *c)
{
    for (; c; c = c->li_next) {
        ...
        while ((gnd = strstr(gnd, "gnd")) != NULL) {
            if ((isspace(char_to_int(gnd[-1])) || gnd[-1] == '(' || gnd[-1] == ',') &&
                (isspace(char_to_int(gnd[3])) || gnd[3] == ')' || gnd[3] == ',')) {
                memcpy(gnd, " 0 ", 3);
            }
            gnd += 3;
        }
...
inp_fix_macro_param_func_paren_io(struct line *card)
{
    ...
    for (; card; card = card->li_next) {
        ...
        if (ciprefix(".param", card->li_line)) {
            ...
            str_ptr = skip_ws(str_ptr);
            while (!isspace(char_to_int(*str_ptr)) && *str_ptr != '=') {
                if (*str_ptr == '(')
                    is_func = TRUE;
                str_ptr++;
            }
...
get_model_name(char *line, int num_terminals)
{
    ...
    if (*line == 'r')           /* special dealing for r models */
        if ((*beg_ptr == '+') || (*beg_ptr == '-') || isdigit(char_to_int(*beg_ptr))) { /* looking for a value before model */
...
is_a_modelname(const char *s)
{
    ...
    if (isalpha(char_to_int(s[0])))
        return TRUE;

    /* e.g. 1N4002 */
    if (isdigit(char_to_int(s[0])) && isalpha(char_to_int(s[1])) && isdigit(char_to_int(s[2])))
        return TRUE;

    /* e.g. 2SK456 */
    if (isdigit(char_to_int(s[0])) && isalpha(char_to_int(s[1])) && isalpha(char_to_int(s[2])) && isdigit(char_to_int(s[3])))
        return TRUE;

    /* e.g. 1SMB4148 */
    if (isdigit(char_to_int(s[0])) && isalpha(char_to_int(s[1])) && isalpha(char_to_int(s[2])) && isalpha(char_to_int(s[3])) && isdigit(char_to_int(s[4])))
        return TRUE;
    ...
    if (string)
        while (*string) {
            ...
            *string = (char) strip(*string);
            ...
            if (!isspace(char_to_int(*string)) && !isprint(char_to_int(*string)))
                *string = '_';
...
inp_fix_subckt(struct names *subckt_w_params, char *s)
{
    ...
    if (equal && !strstr(s, "params:")) {
        ...
        ptr1 = skip_non_ws(s);
        ptr1 = skip_ws(ptr1);
        for (ptr2 = ptr1; *ptr2 && !isspace(char_to_int(*ptr2)) && !isquote(*ptr2); ptr2++)
            ;
...
expand_section_ref(struct line *c, char *dir_name)
{
    ...
    if (*y) {
        ...
        for (z = y; *z && !isspace(char_to_int(*z)) && !isquote(*z); z++)
            ;
        keep_char1 = *t;
        keep_char2 = *z;
        *t = '\0';
...
inp_get_params(char *line, char *param_names[], char *param_values[])
{
    ...
    while ((equal_ptr = find_assignment(line)) != NULL) {
        ...
        *end = '\0';
        ...
        if (*value == '{' ||
            isdigit(char_to_int(*value)) ||
            (*value == '.' && isdigit(char_to_int(value[1]))))
        {
            value = copy(value);
        } else {
...
inp_get_func_from_line(struct function_env *env, char *line)
{
    ...
    while (*end && !isspace(char_to_int(*end)) && *end != '(')
        end++;
    ...
    for (;;)  {
        char *beg = end;
        while (*end && !isspace(char_to_int(*end)) && *end != ',' && *end != ')')
            end++;
        if (end == beg)
            break;
...
search_func_arg(char *str, struct function *fcn, int *which, char *str_begin)
{
    for (; (str = strpbrk(str, fcn->accept)) != NULL; str++) {
        ...
        if (is_arith_char(before) || isspace(char_to_int(before)) || strchr(",=", before)) {
            int i;
            for (i = 0; i < fcn->num_parameters; i++) {
                size_t len = strlen(fcn->params[i]);
                if (strncmp(str, fcn->params[i], len) == 0) {
                    char after = str[len];
                    if (is_arith_char(after) || isspace(char_to_int(after)) || strchr(",=", after)) {
                        *which = i;
                        return str;
                    }
                }
...
inp_do_macro_param_replace(struct function *fcn, char *params[])
{
    ...
    while ((arg_ptr = search_func_arg(arg_ptr, fcn, &i, str)) != NULL) {
        ...
        for (p = arg_ptr; --p > str; )
            if (*p == '(' || *p == ')') {
                if ((*p == '(') && strchr("vi", p[-1]) &&
                    (p - 2 < str || is_arith_char(p[-2]) || isspace(char_to_int(p[-2])) || strchr(",=", p[-2])))
                    is_vi = 1;
                break;
            }
...
inp_expand_macro_in_str(struct function_env *env, char *str)
{
    ...
    while ((open_paren_ptr = strchr(search_ptr, '(')) != NULL) {
        ...
        /* function name consists of numbers, letters and special characters (VALIDCHARS) */
            if (!isalnum(char_to_int(*fcn_name)) && !strchr(VALIDCHARS, *fcn_name))
                break;
        fcn_name++;
        ...
        *open_paren_ptr = '(';
        ...
        for (num_params = 0; curr_ptr < close_paren_ptr; curr_ptr++) {
            char *beg_parameter;
            int num_parens;
            if (isspace(char_to_int(*curr_ptr)))
                continue;
...
inp_fix_param_values(struct line *c)
{
    ...
    for (; c; c = c->li_next) {
        ...
        while ((equal_ptr = find_assignment(line)) != NULL) {
            ...
            if (isdigit(char_to_int(*beg_of_str)) ||
                *beg_of_str == '{' ||
                *beg_of_str == '.' ||
                *beg_of_str == '"' ||
                ((*beg_of_str == '-' || *beg_of_str == '+') && isdigit(char_to_int(beg_of_str[1]))) ||
                ((*beg_of_str == '-' || *beg_of_str == '+') && beg_of_str[1] == '.' && isdigit(char_to_int(beg_of_str[2]))) ||
                ...
                ciprefix("false", beg_of_str))
            {
                ...
                for (;;) {
                    ...
                    if (isdigit(char_to_int(*natok)) || *natok == '{' || *natok == '.' ||
                        *natok == '"' || (*natok == '-' && isdigit(char_to_int(natok[1]))) ||
                        ...
                        eq(natok, "<") || eq(natok, ">"))
                    {
                        ...
                        if (isdigit(char_to_int(natok[1])) ||
                            (natok[1] == '-' && isdigit(char_to_int(natok[2]))))
                        {
                            (void) sprintf(buffer, "%s", natok);
                        } else {
                            *natok = '{';
                        ...
                        if (isdigit(char_to_int(*natok)) || (*natok == '-' && isdigit(char_to_int(natok[1])))) {
                            (void) sprintf(buffer, "%s", natok);
                        } else {
                ...
                *equal_ptr  = '\0';
                ...
                parens = 0;
                while (*end_of_str != '\0' &&
                       (!isspace(char_to_int(*end_of_str)) || (parens > 0)))
                {
                    if (*end_of_str == '(')
                        parens++;
                    if (*end_of_str == ')')
...
get_number_terminals(char *c)
{
    ...
    case 'q': /* recognition of 3/4 terminal bjt's needed */
        ...
        for (k = i; k > i-j-1; k--) {
            ...
            while (*nametmp) {
                if (isalpha(char_to_int(*nametmp)) || (*nametmp == ','))
                    only_digits = FALSE;
                nametmp++;
            }
    ...
    for (; card; card = card->li_next) {
        ...
        if (ciprefix(".param", curr_line)) {
            ...
            while ((equal_ptr = find_assignment(curr_line)) != NULL) {
                ...
                while (*end_param != '\0' && (!isspace(char_to_int(*end_param)) || get_expression || get_paren_expression)) {
                    if (*end_param == '{')
                        get_expression = TRUE;
                    if (*end_param == '(')
...
{
    return (c == '_') || isalnum(char_to_int(c));
}
...
ya_search_identifier(char *str, const char *identifier, char *str_begin)
{
    while ((str = strstr(str, identifier)) != NULL) {
        ...
        if (is_arith_char(before) || isspace(char_to_int(before)) || (str <= str_begin)) {
            char after = str[strlen(identifier)];
            if ((is_arith_char(after) || isspace(char_to_int(after)) || after == '\0'))
                break;
        }
    ...
    for (; card; card = card->li_next) {
        ...
        */
            ...
            if (str_ptr) {
                ...
                if (str_ptr[3] && (isspace(char_to_int(str_ptr[3])) || (str_ptr[3] == '='))) {
    ...
    for (; c && c != end_c; c = c->li_next) {
        ...
        for (i = 0; i < num_params; i++) {
            ...
            while ((s = ya_search_identifier(s, deps[i].param_name, curr_line)) != NULL) {
                ...
                if ((isspace(char_to_int(s[-1])) || s[-1] == '=') &&
                    (isspace(char_to_int(*rest)) || *rest == '\0' || *rest == ')'))
                {
                    int prefix_len;

                    if (isspace(char_to_int(s[-1]))) {
                        s = skip_back_ws(s);
                        if (s[-1] == '{')
                            s--;
                    }

                    if (isspace(char_to_int(*rest))) {
