...
inp_list(FILE *file, struct line *deck, struct line *extras, int type)
{
    ...
    top1:
        for (here = deck; here; here = here->li_next) {
            if (renumber)
                here->li_linenum = i;
            if (ciprefix(".end", here->li_line) && !isalpha(char_to_int(here->li_line[4])))
                continue;
            if (*here->li_line != '*') {
 ...
 */
{
    ...
    if (comfile) {
        ...
        for (dd = deck->li_next; dd; dd = ld->li_next) {
            ...
            for (s = name; *s && isspace(char_to_int(*s)); s++)
                ;
            for (t = s; *t && !isspace(char_to_int(*t)); t++)
                ;
            *t = '\0';
    ...
    )
{
    ...
    if (!noparse) {
        ...
        for (; options; options = options->li_next) {
            for (s = options->li_line; *s && !isspace(char_to_int(*s)); s++)
                ;
...
create_circbyline(char *line)
{
    ...
    if (linec < memlen) {
        if (ciprefix(".end", line) && (line[4] == '\0' || isspace(char_to_int(line[4])))) {
    ...
    for (; card; card = card->li_next) {
        ...
        if (prefix(".model", curr_line)) {
            ...
            while ((end_tstr = beg_tstr = strstr(beg_tstr, "temper")) != NULL) {
                ...
                while(isspace(char_to_int(*beg_pstr)))
                    beg_pstr--;
                while(!isspace(char_to_int(*beg_pstr)))
                    beg_pstr--;
                /* get parameter name */
                ...
                if (*end_tstr == '=') {
                    end_tstr--;
                    while(isspace(char_to_int(*end_tstr)))
                        end_tstr--;
                    while(!isspace(char_to_int(*end_tstr)))
                        end_tstr--;
                }
