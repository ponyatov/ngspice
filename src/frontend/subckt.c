...
inp_subcktexpand(struct line *deck) {
    ...
    for (c = deck; c; c = c->li_next) {    /* iterate on lines in deck */
        ...
        if (ciprefix(start, s)) {   /* if we find .subckt . . . */
            ...
            while (*s && !isspace(char_to_int(*s))) /* skip first token */
                s++;
            while (*s && isspace(char_to_int(*s))) /* skip whitespace */
                s++;
...
finishLine(struct bxx_buffer *t, char *src, char *scname)
{
    ...
    while (*src) {
        ...
        if (((*src != 'v') && (*src != 'V') &&
             (*src != 'i') && (*src != 'I')) ||
            lastwasalpha) {
            lastwasalpha = isalpha(char_to_int(*src));
            bxx_putc(t, *src++);
            continue;
        }
        for (s = src + 1; *s && isspace(char_to_int(*s)); s++)
            ;
        if (!*s || (*s != '(')) {
            lastwasalpha = isalpha(char_to_int(*src));
            bxx_putc(t, *src++);
        ...
        src = s;
        bxx_putc(t, *src++);
        while (isspace(char_to_int(*src)))
            src++;
        for (buf = src; *src && !isspace(char_to_int(*src)) && *src != ',' && *src != ')'; )
            src++;
        buf_end = src;
        ...
        if ((which == 'v') || (which == 'V')) {
            while (*src && (isspace(char_to_int(*src)) || *src == ',')) {
                src++;
            }
            if (*src && *src != ')') {
                for (buf = src; *src && !isspace(char_to_int(*src)) && (*src != ')'); )
                    src++;
...
numnodes(char *name, struct subs *subs, wordlist const *modnames)
{
    ...
    while (*name && isspace(char_to_int(*name)))
        name++;
